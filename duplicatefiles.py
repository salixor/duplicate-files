...
# A simple pager (because there can be a lot of duplicate files)
...
# A function to remove the empty directories we may have left
def remEmptyDir(mypath):
    for root, dirs, files in os.walk(mypath):
         for name in dirs:
             fname = os.path.join(root, name)
             if not os.listdir(fname): # To check whether the dir is empty
                 os.removedirs(fname)
                 remEmptyDir(mypath)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        errorMessage("The script requires a path to run.")
    ...
    # Let's retrieve every file recursively
    for root, dirs, files in os.walk(path):
        for fil in files:
            f.append( os.path.join(root, fil) )

    f.sort() # Just a bit of sorting
    # f.sort(reverse=True) # Just a bit of sorting
    ...
    for i, file1 in enumerate(f):
        ...
        for j, file2 in enumerate(f):
            ...
            count += 1
            f1_name = os.path.abspath( os.path.join(path, file1) )
            f2_name = os.path.abspath( os.path.join(path, file2) )
    ...
    if(len(l) > 0):
        ...
        for index, duo in enumerate(l):
            ...
            dupes_list += "\n  The file          \033[33m"+os.path.relpath(duo[1], path)+"\033[39m"
            dupes_list += "\n  is a duplicate of \033[33m"+os.path.relpath(duo[0], path)+"\033[39m"
    ...
    if delete and len(l) > 0:
        ...
        # Proceeds to the deletion of duplicate files (if the user really wishes to delete)
        if check_deletion.lower() == "y":
            for fname in l:
                file_to_remove = os.path.abspath( os.path.join(path, fname[1]) )
                try:
                    os.remove(file_to_remove)
                    print("\033[1;31mDeleted\033[0;39m "+file_to_remove)
                except OSError:
                    errorMessage("The file "+file_to_remove+" does not seem to exist any longer ... Skipping it.")

            remEmptyDir(path) # Let's remove empty directories
            confirmationMessage("\nThe duplicate files were deleted.")
        # If the user cancels its choice, the script does nothing
        else:
            message("The script will now exit without doing anything.")
